...
func TestTransactionPool_Push(t *testing.T) {
	...
	txPool := NewTransactionPoolLogic(nil, 128000)
	txPool.Push(tx1)
	...
	newTxPool := NewTransactionPoolLogic(nil, 128000)
...
func TestTransactionPool_addTransaction(t *testing.T) {
	...
	txPool := NewTransactionPoolLogic(nil, 128)
	...
	assert.Equal(t, 1, len(txPool.txpool.Txs))
	assert.Equal(t, 1, len(txPool.txpool.TipOrder))
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[0])
	...
	assert.Equal(t, 2, len(txPool.txpool.Txs))
	assert.Equal(t, 1, len(txPool.txpool.TipOrder))
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[0])
	...
	assert.Equal(t, 3, len(txPool.txpool.Txs))
	assert.Equal(t, 1, len(txPool.txpool.TipOrder))
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[0])
	...
	assert.Equal(t, 4, len(txPool.txpool.Txs))
	assert.Equal(t, 1, len(txPool.txpool.TipOrder))
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[0])

	//push ttx4. It should be stored in txs and tipOrder
	txPool.addTransactionAndSort(transaction.NewTransactionNode(txs[4]))
	assert.Equal(t, 5, len(txPool.txpool.Txs))
	assert.Equal(t, 2, len(txPool.txpool.TipOrder))
	//since ttx4 has a higher tip than ttx0, it should rank position 0 in tipOrder
	assert.Equal(t, hex.EncodeToString(txs[4].ID), txPool.txpool.TipOrder[0])
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[1])
	...
	assert.Equal(t, 6, len(txPool.txpool.Txs))
	assert.Equal(t, 2, len(txPool.txpool.TipOrder))
	//since ttx4 has a higher tip than ttx0, it should rank position 0 in tipOrder
	assert.Equal(t, hex.EncodeToString(txs[4].ID), txPool.txpool.TipOrder[0])
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[1])
	...
	assert.Equal(t, 7, len(txPool.txpool.Txs))
	assert.Equal(t, 3, len(txPool.txpool.TipOrder))
	//since ttx4 has a higher tip than ttx0, it should rank position 0 in tipOrder
	assert.Equal(t, hex.EncodeToString(txs[6].ID), txPool.txpool.TipOrder[0])
	assert.Equal(t, hex.EncodeToString(txs[4].ID), txPool.txpool.TipOrder[1])
	assert.Equal(t, hex.EncodeToString(txs[0].ID), txPool.txpool.TipOrder[2])
	assert.Equal(t, uint32(339), txPool.txpool.CurrSize)
}
...
func TestTransactionPool_RemoveTransactionNodeAndChildren(t *testing.T) {
	...
	assert.Equal(t, 7, len(txPool.txpool.Txs))
	assert.Equal(t, uint32(437), txPool.txpool.CurrSize)
	//Since tx0 is the root, all txs wlil be removed
	txPool.removeTransactionNodeAndChildren(txs[0])
	assert.Equal(t, 4, len(txPool.txpool.Txs))
	assert.Equal(t, uint32(300), txPool.txpool.CurrSize)
}
...
func TestTransactionPool_removeMinTipTx(t *testing.T) {
	...
	assert.Equal(t, hex.EncodeToString(txs[6].ID), txPool.txpool.TipOrder[0])
	assert.Equal(t, hex.EncodeToString(txs[4].ID), txPool.txpool.TipOrder[1])
	assert.Equal(t, hex.EncodeToString(txs[7].ID), txPool.txpool.TipOrder[2])
}
...
func TestTransactionPool_Update(t *testing.T) {
	...
	assert.Equal(t, 7, len(txPool.txpool.Txs))
	assert.Equal(t, 5, len(txPool.txpool.TipOrder))
	assert.Equal(t, hex.EncodeToString(txs[6].ID), txPool.txpool.TipOrder[0])
	assert.Equal(t, hex.EncodeToString(txs[4].ID), txPool.txpool.TipOrder[1])
	assert.Equal(t, hex.EncodeToString(txs[1].ID), txPool.txpool.TipOrder[2])
	assert.Equal(t, hex.EncodeToString(txs[7].ID), txPool.txpool.TipOrder[3])
	assert.Equal(t, hex.EncodeToString(txs[2].ID), txPool.txpool.TipOrder[4])
}

func TestTransactionPoolLimit(t *testing.T) {
	txPool := NewTransactionPoolLogic(nil, 0)
	txPool.Push(tx1)
	assert.Equal(t, 0, len(txPool.GetTransactions()))

	txPool = NewTransactionPoolLogic(nil, 1)
	txPool.Push(tx1)
...
func TestTransactionPool_GetTransactions(t *testing.T) {
	...
	txPool := NewTransactionPoolLogic(nil, 100000)
...
func TestTransactionPool_Rollback(t *testing.T) {
	...
	*/
	...
	assert.Equal(t, tx0Id, txPool.txpool.TipOrder[0])
	assert.Equal(t, 2, len(txPool.txpool.Txs[tx0Id].Children))
}
...
func TestTransactionPool_Proto(t *testing.T) {
	...
	txpooldata := transactionpool.NewTransactionPool()
	txPool1 := NewTransactionPoolLogic(nil, 128)
	txpooldata.FromProto(txPoolProto)
	txPool1.txpool = txpooldata
	assert.Equal(t, txPool.pendingTxs, txPool1.pendingTxs)
	assert.Equal(t, txPool.txpool.TipOrder, txPool1.txpool.TipOrder)
	assert.Equal(t, txPool.txpool.Txs, txPool1.txpool.Txs)
	assert.Equal(t, txPool.txpool.CurrSize, txPool1.txpool.CurrSize)
